...
const getAnchorCoordinates = ({ x, y, width, height }) => {
  return [
    // top left
    { x, y, priority: 0.8 },
    // top center
...
const getClosestAnchors = (anchor1, anchor2) => {
...
const getChildElementsByName = (node, localName) => {
...
const testRequiredAttribute = (elementName, attributeName, value) => {
...
const parseChildElements = (doc, parent) =>
  ...
  connectedCallback() {
    ...
    fetch(url).then((response) => {
      response.text().then((svg) => {
        ...
        let allVakjes = parseChildElements(doc, x);
        ...
        let vakjes = (x ? getChildElementsByName(x, "child") : [])
          .map((x) => parseChild(doc, x))
          .filter((x) => x !== null);
        ...
        const connections = (x ? getChildElementsByName(x, "child") : [])
          .map((child) => {
            return (
        ...
        const renderChildren = (vakjes, zIndex = 1) => {
